...
def buscar_detalhes_xid(xid):
    ...
    print("Sem cache - procurando na API")  # Log para indicar que será feita uma requisição
    ...
    if response.status_code == 200:
        ...
        redis_client.setex(cache_key, 86400, json.dumps(detalhes))  # Armazena no cache por 1 dia
        return detalhes
    else:
...
def get_coords(city):
    query = f'geoname?name={city}&apikey={API_KEY}'  # Monta a URL da requisição
    url = base + query
    response = requests.get(url).json()  # Faz a requisição e converte para JSON
    return response.get('lat'), response.get('lon')  # Retorna latitude e longitude

# Função para buscar pontos turísticos próximos a uma coordenada
def get_places(lat, lon):
    query = f'radius?radius=2000&lon={lon}&lat={lat}&rate=2&format=json&'  # Define parâmetros da busca
...
def pontos_turisticos():
    ...
    if not cidade:
        return jsonify({'erro': 'Endereço não fornecido'}), 400  # Retorna erro se não houver cidade

    lat, lon = get_coords(cidade)  # Obtém coordenadas da cidade
    if not lat or not lon:
        return jsonify({'erro': 'Localização não encontrada'}), 400  # Retorna erro se não encontrar coordenadas
...
def detalhes():
    ...
    if not xid:
        return jsonify({'erro': 'XID não fornecido'}), 400  # Retorna erro se não houver xid

    dados = buscar_detalhes_xid(xid)  # Busca os detalhes usando o xid
    if not dados:
        return jsonify({'erro': 'Detalhes não encontrados'}), 404  # Retorna erro se não encontrar dados
